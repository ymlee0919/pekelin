...
const VariantsTBar = (props : VariantsTBarProps) => {
    ...
    <div className="navbar bg-gray-200 min-h-1 p-1">
        <div className="flex-1">
            <Link to={RouterTable.variants.new(props.selectedItem?.productId || 0)} className="btn btn-ghost text-slate-500 btn-sm text-sm mr-2 rounded-none">
                <MdOutlineAdd /> Add
            </Link>
                
            <Link to={RouterTable.variants.edit(props.selectedItem?.productId || 0, props.selectedItem?.variantId || 0)} className={`btn btn-ghost text-slate-500 btn-sm text-sm mx-2 rounded-none ${
